use cfd_rs_utils::mesh::{assembled_mesh::Mesh, computational_mesh::{manual_meshes::quad_square, Computational2DMesh}};
...
fn main() {
    ...
    let mesh = Computational2DMesh::deserialize_file("/home/xaviern/Documents/VSCode/CFD/cfd-rs-workspace/cfd-rs/target/exports/mesh.cfd").unwrap();
    let assembled = Mesh::from(mesh);
    assembled.export_cell_centered("./out/test.vtu".to_owned()).unwrap();
    assembled.export_node_centered("./out/test_node.vtu".to_owned()).unwrap();
}
