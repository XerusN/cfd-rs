    ...
    pub fn discretize(&self, system: &mut System, fields: &Vec<RefMut<Field>>, schemes: &Schemes, coeff: f64) {
        match self {
            Self::Laplacian(var, integration) => {
                schemes.laplacian.discretize(&var, system, &integration, fields, coeff);
            }
            Self::Convection {var, integration, speed } => schemes.convection.discretize(&var, &speed, system, &integration, fields, coeff),
            Self::Divergence(var, integration) => schemes.divergence.discretize(&var, system, &integration, fields, coeff),
            Self::TimeDerivative(var) => schemes.transient.discretize(&var, system, fields, coeff),
        }
    }
}

fn find_var_in_fields<'a, 'b>(var: &Variable, system: &System, fields: &'a Vec<RefMut<'b, Field>>) -> &'a RefMut<'b, Field> {
    let index = system.fields_required().iter().position(|var_i| var == var_i).expect(&format!("Missing variable {var:?} in fields for equation {:?}", system.equation()));
    &fields[index]
}